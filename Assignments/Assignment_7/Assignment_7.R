...
utah2 <- arrange(utah, desc(Pop_2010))
...
df1 <- utah %>% 
  group_by(Pop.Group) %>% summarise(Mean_Religious=mean(Religious))
...
df2 <- utah %>% 
  group_by(Pop.Group) %>% summarise(Mean_Pop_2010=mean(Pop_2010))
...
# They are inversely related.  For the LDS negative correlation, it provides evidence that in a given area, if someone isn't LDS, they probably aren't religious.  
# For Episcopal Church, the plot shows a relationship of those church members living in an area where there are more nonreligious people.
