...
ggplot(midwest) # what do you see? --> Blank white screen pops up under "Plots" tab
...
# Zoom in
p + lims(x=c(0,0.1),y=c(0,1000000)) # what did this do? Changed the X and Y axis value limits
p + coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) # how is this different? It changed the best fit line.  
...
library(tidyverse)
png("iris_fig3.png")
iris2 <- subset(iris, select = c("Petal.Width","Sepal.Width","Species"))
iris2$Ratio <- (iris2$Petal.Width / iris2$Sepal.Width)
ggplot(iris2,aes(x=Species,y=Ratio,fill=Species)) + 
  geom_boxplot() +
...
data("iris")
iris$Ratio <- rownames(iris)
iris$Ratio_score <- round((iris$Sepal.Length - 
                             mean(iris$Sepal.Length))
                           /sd(iris$Sepal.Length),digits=2)

iris$Ratio_type <- ifelse(iris$Ratio_score < 0, "below","above")
iris <- iris[order(iris$Ratio_score), ]
iris$Ratio <- factor(iris$Ratio, levels = iris$Ratio)
